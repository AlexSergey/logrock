...
describe('LimitedArray', () => {
  ...
  describe('positive cases', () => {
    it('initializes with the given limit', () => {
      const arr = new LimitedArray<number>({ limit: 20 });
    ...
    it('returns true from add when the limit is exceeded', () => {
      const arr = new LimitedArray<number>({ limit: 2 });
      arr.add(1);
      arr.add(2);
    ...
    it('evicts the oldest item when the limit is exceeded by one', () => {
      const arr = new LimitedArray<number>({ limit: 3 });
      arr.add(1);
      arr.add(2);
      arr.add(3);
      arr.add(4);
    ...
    it('setLimit uses the absolute value for negative numbers', () => {
      const arr = new LimitedArray<number>({ limit: 10 });
      arr.setLimit(-3);
