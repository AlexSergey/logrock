...
const createWrapper =
  ...
  ({ children }: PropsWithChildren) =>
    <LoggerContainer {...props}>{children}</LoggerContainer>;
...
describe('LoggerContainer', () => {
    ...
    it('leaves sessionId undefined when sessionID prop is a boolean', () => {
      const { result } = renderHook(() => useLoggerApi(), {
        wrapper: ({ children }: PropsWithChildren) => (
          <LoggerContainer sessionID={false}>{children}</LoggerContainer>
        ),
      });
  ...
  describe('positive cases', () => {
    ...
    it('triggerError invokes onError with an IStack', () => {
      const onError = jest.fn();
      ...
      expect(onError).toHaveBeenCalledTimes(1);
      expect(onError.mock.calls[0]![0]).toHaveProperty('actions');
    });
    ...
    it('sets sessionId when sessionID prop is a number', () => {
      const { result } = renderHook(() => useLoggerApi(), {
        wrapper: ({ children }: PropsWithChildren) => (
          <LoggerContainer sessionID={42}>{children}</LoggerContainer>
        ),
      });
    ...
    it('calls onError when a window error event fires', () => {
      ...
      expect(onError).toHaveBeenCalledTimes(1);
      const stackData = onError.mock.calls[0]![0] as IStack;
      expect(stackData.actions.some((a) => 'critical' in a)).toBe(true);
    });

    it('uses the custom bsod component when the bsod prop is provided', () => {
      const CustomBsod = ({ count }: IBSOD) => <div data-testid="custom-bsod">count:{count}</div>;
      render(
