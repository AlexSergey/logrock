...
import { getStackData, onCriticalError } from "./stack.mjs";
...
describe('getStackData', () => {
  describe('negative cases', () => {
    it('does not throw when no props are provided', () => {
      expect(() => getStackData(makeStack(), new LimitedArray(), {})).not.toThrow();
    });
    it('does not call onPrepareStack when it is not provided', () => {
      const spy = jest.fn();
      getStackData(makeStack(), new LimitedArray(), {});
    ...
    it('sets env.lang to empty string when navigator is unavailable', () => {
      ...
      const result = getStackData(makeStack(), new LimitedArray(), {});
      Object.defineProperty(globalThis, 'navigator', {
        configurable: true,
        value: original
      });
  ...
  describe('positive cases', () => {
    ...
    it('populates actions from the collection', () => {
      const collection = new LimitedArray();
      collection.add({
        log: 'action-a'
      });
      collection.add({
